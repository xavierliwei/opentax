...
export function analyzeGaps(taxReturn: TaxReturn, computeResult: ComputeResult): GapAnalysisResult {
  ...
  // ── Filing status ────────────────────────────────────────────
  // Filing status always has a default ('single'), so it's technically set,
  // but we flag it as needing confirmation if no personal info is entered yet
  if (!tp.firstName && !tp.lastName) {
    items.push({
  ...
  // Weight: required items = 10 pts each, recommended = 3, optional = 1
