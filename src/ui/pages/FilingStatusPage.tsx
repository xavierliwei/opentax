...
export function FilingStatusPage() {
  ...
  const caResident = useTaxStore((s) => s.taxReturn.caResident ?? false)
  const setCAResident = useTaxStore((s) => s.setCAResident)
  const rentPaidInCA = useTaxStore((s) => s.taxReturn.rentPaidInCA ?? false)
  const setRentPaidInCA = useTaxStore((s) => s.setRentPaidInCA)
      ...
      <div className="mt-6 border border-gray-200 rounded-lg p-4">
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
            checked={caResident}
            onChange={(e) => setCAResident(e.target.checked)}
            className="mt-1"
          />
          <div>
            <span className="font-medium text-gray-900 inline-flex items-center">
              ...
              <InfoTooltip
                explanation="Check this if you lived in California for the entire tax year. OpenTax will compute your California Form 540 state return alongside your federal return. CA taxes all income at ordinary rates (no preferential capital gains rate) and adds a 1% mental health surcharge on taxable income above $1 million."
                pubName="FTB — Filing Requirements"
                pubUrl="https://www.ftb.ca.gov/file/personal/residency-status/index.html"
              />
            </span>
            <p className="text-sm text-gray-500 mt-0.5">
              Enables California Form 540 computation.
            </p>
          </div>
        </label>
        ...
        {caResident && (
          <label className="flex items-start gap-3 cursor-pointer mt-3 ml-6">
            <input
              type="checkbox"
              checked={rentPaidInCA}
              onChange={(e) => setRentPaidInCA(e.target.checked)}
              className="mt-1"
            />
            <div>
              <span className="font-medium text-gray-900 inline-flex items-center">
