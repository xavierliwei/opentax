...
export async function fillForm1040NR(
  ...
  oiLines.push('')
  oiLines.push(`IRC §871(d) election (treat rental as ECI): ${(nra?.rentalElectECI ?? false) ? 'Yes' : 'No'}`)
  oiLines.push(`Social Security treaty exemption: ${(nra?.socialSecurityTreatyExempt ?? false) ? 'Yes' : 'No'}`)
